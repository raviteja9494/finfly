...
fun AssistantScreen(viewModel: AssistantViewModel = hiltViewModel()) {
    ...
    when (val modelState = state.modelState) {
        ...
        is AiModelState.Error -> if (state.messages.isEmpty()) {
            ...
            ChatContent(state, viewModel, onShowInfo = { showInfo = true })
        }
        AiModelState.Loading,
        AiModelState.Ready,
        is AiModelState.Downloaded -> ChatContent(state, viewModel, onShowInfo = { showInfo = true })
    }
}
...
@Composable
private fun ChatContent(state: AssistantUiState, viewModel: AssistantViewModel, onShowInfo: () -> Unit) {
    val spacing = FinFlyThemeTokens.spacing
    Column(Modifier.fillMaxSize()) {
        ...
        if (state.messages.isEmpty()) {
            AssistantWelcome(onSuggestion = viewModel::sendMessage, modifier = Modifier.weight(1f))
        } else {
            LazyColumn(
                ...
                items(state.messages.asReversed(), key = ChatMessage::id) { message -> ChatBubble(message) }
            }
        }
...
@Composable
private fun AssistantWelcome(onSuggestion: (String) -> Unit, modifier: Modifier = Modifier) {
    val suggestions = listOf(
        stringResource(R.string.ai_suggestion_spending),
        stringResource(R.string.ai_suggestion_categories),
        stringResource(R.string.ai_suggestion_balance),
    )
    val spacing = FinFlyThemeTokens.spacing
    Column(
