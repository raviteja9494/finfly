...
import com.teja.finflyiii.presentation.accounts.AccountEditorScreen
import com.teja.finflyiii.presentation.accounts.AccountsScreen
import com.teja.finflyiii.presentation.dashboard.DashboardScreen
import com.teja.finflyiii.presentation.featurelist.FeatureListScreen
import com.teja.finflyiii.presentation.featureeditor.FeatureEditorScreen
import com.teja.finflyiii.presentation.assistant.AssistantScreen
import com.teja.finflyiii.presentation.reports.ReportsScreen
import com.teja.finflyiii.presentation.settings.SettingsScreen
import com.teja.finflyiii.presentation.transactiondetail.TransactionDetailScreen
import com.teja.finflyiii.presentation.transactioneditor.TransactionEditorScreen
import com.teja.finflyiii.presentation.transactions.TransactionsScreen
...
import com.teja.finflyiii.presentation.theme.LocalFinFlyIIIZoneId

private enum class FinFlyIIITab(val label: Int, val icon: ImageVector) {
...
@Composable
fun FinFlyIIIApp(viewModel: AppShellViewModel = hiltViewModel()) {
    ...
    DisposableEffect(lifecycleOwner, viewModel) {
        val observer = LifecycleEventObserver { _, event ->
            if (event == Lifecycle.Event.ON_START) viewModel.sync()
        }
    ...
    ModalNavigationDrawer(
        ...
        Scaffold(
            ...
            CompositionLocalProvider(
                LocalFinFlyIIIZoneId provides if (useDeviceZone) ZoneId.systemDefault() else ZoneOffset.UTC,
            ) {
                NavHost(
...
private fun NavHostController.navigateTab(tab: FinFlyIIITab) {
    val startId = graph.findStartDestination().id
    when (tab) {
        FinFlyIIITab.DASHBOARD -> navigate(AppRoute.Dashboard) {
            configureTabNavigation(startId)
        }
        FinFlyIIITab.TRANSACTIONS -> navigate(AppRoute.Transactions()) {
            configureTabNavigation(startId)
        }
        FinFlyIIITab.REPORTS -> navigate(AppRoute.Reports) {
            configureTabNavigation(startId)
        }
        FinFlyIIITab.ASSISTANT -> navigate(AppRoute.Assistant) {
            configureTabNavigation(startId)
        }
    }
}
...
@Composable
private fun FinFlyIIIBottomBar(destination: NavDestination?, onSelect: (FinFlyIIITab) -> Unit) {
    NavigationBar(containerColor = MaterialTheme.colorScheme.surface, tonalElevation = 2.dp) {
        FinFlyIIITab.entries.forEach { tab ->
            val selected = destination?.hierarchy?.any { item ->
                when (tab) {
                    FinFlyIIITab.DASHBOARD -> item.hasRoute<AppRoute.Dashboard>()
                    FinFlyIIITab.TRANSACTIONS -> item.hasRoute<AppRoute.Transactions>()
                    FinFlyIIITab.REPORTS -> item.hasRoute<AppRoute.Reports>()
                    FinFlyIIITab.ASSISTANT -> item.hasRoute<AppRoute.Assistant>()
                }
            } == true
            NavigationBarItem(
                selected = selected,
